...
if (
  ...
  !["55439", "55440"].includes(url.port) ||
  url.pathname !== "/acceptance_test"
) {
...
async function party(lower = 100_000, upper: number | null = 100_000) {
  ...
  const [org] = await db
    ...
    .values({
      type: "PHARMACY",
      status: "APPROVED",
      legalNameEncrypted: "test-only",
      publicAlias: "Test Pharmacy",
      province: "Kars",
      district: "Merkez",
      creditLimitKurus: lower,
      creditUpperLimitKurus: upper
    })
    .returning();
  await db
    .insert(schema.organizationMembers)
    .values({ userId: user.id, organizationId: org.id, role: "ORGANIZATION_OWNER" });
  ...
  const actor: AppSessionUser = {
    ...
    organizationRoles: { [org.id]: ["ORGANIZATION_OWNER"] },
    totpEnabled: false
  };
...
async function fixture(lower = 100_000, upper: number | null = 100_000) {
  ...
  const [product] = await db
    ...
    .values({ name: "Fixture medicine", type: "HUMAN", gtin: randomUUID().replaceAll("-", "") })
    .returning();
  ...
  const [listing] = await db
    ...
    .values({
      sellerOrganizationId: seller.org.id,
      batchId: batch.id,
      status: "ACTIVE",
      unitReferenceValueKurus: 1000,
      quantityAvailable: 100,
      minExpiryDate: "2030-01-01"
    })
    .returning();
...
async function balance(accountId: string) {
  const [row] = await db
    .select({
      value: sql<number>`coalesce(sum(case when direction='CREDIT' then amount_kurus else -amount_kurus end),0)::bigint`
    })
...
function credit(payload: Record<string, unknown>) {
  return POST(
    new NextRequest("http://localhost:3000/api/admin/credit-limits", {
      method: "POST",
...
beforeAll(async () => {
  const [user] = await db
    .insert(schema.users)
    .values({ name: "Test admin", email: `${randomUUID()}@example.invalid` })
    .returning();
  admin = {
    id: user.id,
    email: user.email,
...
describe("real PostgreSQL accounting", () => {
  it("isolates private conversations and only marks explicitly displayed messages read", async () => {
    ...
    const { markMessageNotificationsRead, getNavigationBadgeCounts, deleteUserNotification } =
      await import("@/modules/notifications/service");
    const a = await party();
    const b = await party();
    const outsider = await party();
    const first = await sendOrganizationMessage({
      actor: a.actor,
      organizationId: a.org.id,
      recipientOrganizationId: b.org.id,
      body: "Private fixture first"
    });
    const second = await sendOrganizationMessage({
      actor: a.actor,
      organizationId: a.org.id,
      conversationId: first.conversationId,
      body: "Private fixture second"
    });
    await expect(
      listConversationMessages(outsider.actor, outsider.org.id, first.conversationId)
    ...
    const [secondNotification] = await db
      ...
      .where(
        and(
          eq(schema.notifications.userId, b.actor.id),
          eq(schema.notifications.messageId, second.id)
        )
      );
    await expect(
      deleteUserNotification(outsider.actor.id, { notificationId: secondNotification.id })
    ).rejects.toThrow("NOTIFICATION_NOT_FOUND");
    await deleteUserNotification(b.actor.id, { notificationId: secondNotification.id });
    expect((await getNavigationBadgeCounts(b.actor.id)).messages).toBe(0);
    expect(
      (await listConversationMessages(b.actor, b.org.id, first.conversationId)).map((m) => m.body)
    ).toEqual(["Private fixture first", "Private fixture second"]);
  });
  it("allows debt, settles once, preserves stock and opposite ledger entries", async () => {
    const f = await fixture();
    const input = {
      buyerOrganizationId: f.buyer.org.id,
      listingId: f.listing.id,
      quantity: 20,
      idempotencyKey: randomUUID()
    };
    const reserved = await Promise.all([
      createOrderReservation(f.buyer.actor, input),
      createOrderReservation(f.buyer.actor, input)
    ]);
    ...
    expect(
      (
        await db
          ...
          .where(
            and(
              eq(schema.notifications.userId, f.seller.actor.id),
              eq(schema.notifications.type, "ORDER_RECEIVED")
            )
          )
      ).length
    ).toBe(1);
    ...
    expect(
      (
        await db
          ...
          .where(
            and(
              eq(schema.notifications.userId, f.buyer.actor.id),
              eq(schema.notifications.type, "ORDER_HANDOVER")
            )
          )
      ).length
    ).toBe(1);
    await Promise.allSettled([
      confirmBuyerDelivery(f.buyer.actor, reserved[0].id),
      confirmBuyerDelivery(f.buyer.actor, reserved[0].id)
    ]);
    expect(
      (
        await db
          ...
          .where(
            and(
              eq(schema.notifications.userId, f.seller.actor.id),
              eq(schema.notifications.type, "ORDER_COMPLETED")
            )
          )
      ).length
    ).toBe(1);
    ...
    const [batch] = await db
      ...
      .where(eq(schema.productBatches.id, f.batch.id));
    expect([batch.availableQuantity, batch.reservedQuantity, batch.transferredQuantity]).toEqual([
      80, 0, 20
    ]);
  });
  it("serializes concurrent seller limit reservations", async () => {
    ...
    const results = await Promise.allSettled(
      [1, 2].map(() =>
        createOrderReservation(f.buyer.actor, {
          buyerOrganizationId: f.buyer.org.id,
          listingId: f.listing.id,
          quantity: 20,
          idempotencyKey: randomUUID()
  ...
  it("enforces lower limits and releases cancellation holds only once", async () => {
    ...
    ).rejects.toThrow("Insufficient takas balance");
    const order = await createOrderReservation(f.buyer.actor, {
      buyerOrganizationId: f.buyer.org.id,
      listingId: f.listing.id,
      quantity: 10,
      idempotencyKey: randomUUID()
    ...
    await Promise.allSettled([
      cancelOrderReservation(f.buyer.actor, order.id),
      cancelOrderReservation(f.buyer.actor, order.id)
    ]);
    const [listing] = await db
      .select()
  ...
  it("changes limits without money, deduplicates adjustment and rejects unauthorized admin writes", async () => {
    ...
    const [org] = await db
      ...
      .where(eq(schema.organizations.id, p.org.id));
    expect(org.creditUpperLimitKurus).toBeNull();
    const input = {
      organizationId: p.org.id,
      operation: "ADJUST_BALANCE",
      balanceDelta: 100,
      reason: "Acceptance adjustment test",
      idempotencyKey: randomUUID()
    };
