...
function collectEvidence(formData: FormData) {
  return [
    ...
    return isProvidedFile(file)
      ? [{ kind: kind as "image" | "invoice" | "package" | "other", file }]
      : [];
  });
}
...
async function handlePost(request: NextRequest) {
  ...
  try {
    ...
    const input = formData
      ? parseListingSubmissionFormData(formData)
      : parseListingSubmission(await request.json());
    const result = await submitListingForReview(
      actor,
      input,
      formData ? collectEvidence(formData) : []
    );

    if (formData && request.headers.get("accept")?.includes("text/html")) {
      return NextResponse.redirect(new URL(`/ilan-olustur?submitted=${result.id}`, request.url), {
        status: 303
      });
    }
...
export const POST = mutationRoute("src/app/api/listings", handlePost);
