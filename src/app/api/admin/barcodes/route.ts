...
import { auditLogs, productCatalog, titckSkrsProducts } from "@/lib/db/schema";
  ...
  try {
    ...
    if (input.action === "DELETE") {
      const [updated] = await db
        ...
        .where(and(eq(productCatalog.id, input.productId), eq(productCatalog.source, "MANUAL")))
        .returning({ id: productCatalog.id });
      if (!updated) return NextResponse.json({ error: "NOT_FOUND" }, { status: 404 });
      await db.insert(auditLogs).values({
        actorUserId: actor.id,
        action: "PRODUCT_CATALOG_DISABLED",
    ...
    const [officialProduct] = await db
      .select({ gtin: titckSkrsProducts.gtin })
      .from(titckSkrsProducts)
      .where(and(eq(titckSkrsProducts.gtin, input.gtin), eq(titckSkrsProducts.status, "Aktif")))
      .limit(1);
    if (officialProduct) {
      return NextResponse.json({ error: "BARCODE_MANAGED_BY_TITCK" }, { status: 409 });
    }
    const values = {
      gtin: input.gtin,
      name: input.name,
    ...
    const [saved] = input.productId
      ...
      : await db
          ...
          .values({ ...values, source: "MANUAL", controlCategory: "STANDARD" })
          .returning();
    if (!saved) return NextResponse.json({ error: "NOT_FOUND" }, { status: 404 });
    await db.insert(auditLogs).values({
      actorUserId: actor.id,
      action: input.productId ? "PRODUCT_CATALOG_UPDATED" : "PRODUCT_CATALOG_CREATED",
      targetType: "product_catalog",
      targetId: saved.id,
      safeAfter: {
        gtin: saved.gtin,
      ...
      correlationId: randomUUID(),
      reason: "Super admin managed barcode catalog."
    });
    ...
    if (error && typeof error === "object" && "code" in error && error.code === "23505")
      return NextResponse.json({ error: "BARCODE_ALREADY_EXISTS" }, { status: 409 });
    throw error;
  }
}
