...
import {
  organizationReviewInputSchema,
  parseOrganizationReviewFormData
} from "@/modules/verification/review-input";
...
async function handlePost(request: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      error.message.includes("Invalid organization review transition")
    ) {
      return NextResponse.json(
        { error: "INVALID_ORGANIZATION_REVIEW_TRANSITION" },
        { status: 409 }
      );
    }
...
export const POST = mutationRoute("src/app/api/admin/organization-reviews", handlePost);
