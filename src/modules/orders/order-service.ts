...
function calculateLedgerBalance(entries: LedgerEntryRow[]) {
  return entries.reduce(
    (sum, entry) => (entry.direction === "CREDIT" ? sum + entry.amountKurus : sum - entry.amountKurus),
    0
  );
}
...
export async function createOrderReservation(actor: AppSessionUser, input: OrderCreationInput) {
  ...
  return db.transaction(async (tx) => {
    ...
    const [product] = await tx
      .select({ type: productCatalog.type })
      .from(productCatalog)
...
export async function markSellerHandover(actor: AppSessionUser, orderId: string) {
  ...
  return db.transaction(async (tx) => {
    ...
    if (!["RESERVED", "CONTACT_DETAILS_REVEALED", "SELLER_PREPARING", "READY_FOR_PICKUP"].includes(order.status)) {
...
export async function cancelOrderReservation(actor: AppSessionUser, orderId: string) {
  ...
  return db.transaction(async (tx) => {
    ...
    await tx
      ...
      .where(and(eq(balanceHolds.orderId, orderId), isNull(balanceHolds.releasedAt)));
...
export async function completeOrder(orderId: string, actorUserId: string | null = null) {
  const db = getDb();
  return db.transaction(async (tx) => {
    const [order] = await tx
      .select({
    ...
    const [reservation] = await tx
      .select({ id: inventoryReservations.id, batchId: inventoryReservations.batchId })
      .from(inventoryReservations)
      .where(and(eq(inventoryReservations.orderId, order.id), isNull(inventoryReservations.consumedAt)))
      .limit(1);
    ...
    await tx
      ...
      .where(and(eq(balanceHolds.orderId, order.id), isNull(balanceHolds.consumedAt)));
    await tx
      .update(inventoryReservations)
      .set({ consumedAt: new Date() })
      .where(eq(inventoryReservations.id, reservation.id));
